...
<body>
            ...
            <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
              <div class="hidden sm:ml-6 sm:block">
                <div class="flex space-x-4">
                  ...
                  <a href="#" class="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white" aria-current="page">Prestamos</a>
    ...
    $resultUser = $conexion->query($datosUser);
    ...
    if ($resultado->num_rows > 0) {
        ...
        while ($row = $resultado->fetch_assoc()) { 
            if ($resultUser->num_rows > 0) {
                while($celda = $resultUser->fetch_assoc()){  ?>
                    <ul role="list" class="divide-y divide-gray-100">
                        <li class="flex justify-between gap-x-6 py-5">
                            <div class="flex min-w-0 gap-x-4">
                                <img class="h-12 w-12 flex-none rounded-full bg-gray-50" src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="">
                                <div class="min-w-0 flex-auto">
                                    <p class="text-sm font-semibold leading-6 text-gray-900"> <?php echo $celda['nombre']." ". $celda['apellido'];?></p>
                                    <p class="mt-1 truncate text-xs leading-5 text-gray-500"><?php echo $celda['correo']; ?></p>
                                </div>
                            </div>
                            <div class="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
                                <p class="text-sm leading-6 text-gray-900">fecha prestamo: <?php echo $row['fecha_prestamo'];?> </p>
                                <p class="mt-1 text-xs leading-5 text-gray-500">fecha devolucion: <?php echo $row['fecha_devolucion'];?> </p>
                            </div>
                        </li>
                    </ul>
          <?php }
